"""The bank as the harness sees it: a read-only SQLite file, `make seed`, and
the MCP endpoint that serves the same file.
...
from harness.application.ports.bank import BankEndpoint, BankLifecycle, BankObservation
...
BANK_MCP = Path(__file__).parents[3] / "bank-mcp"
DATA_DIR = Path(__file__).parents[3] / ".data"
PROBE_TIMEOUT_S = 1.0
PING_TIMEOUT_S = 2.0


class McpEndpoint(BankEndpoint):
    """The bank-mcp URL. Probing it never calls a tool that moves money."""

    def __init__(self, url: str, *, timeout_s: float = PROBE_TIMEOUT_S) -> None:
        self.url = url
    ...
    def read_balance(self, account: str) -> None:
        """One `get_balance` on behalf of the account; the bank records it."""
        asyncio.run(self._read_balance(account))
    ...
    async def _read_balance(self, account: str) -> None:
        async with (
            httpx.AsyncClient(
                headers={"X-Account-Id": account}, timeout=self.timeout_s
            ) as http,
    ...
    async def _ping(self) -> None:
        async with (
            httpx.AsyncClient(timeout=PING_TIMEOUT_S) as http,
            streamable_http_client(self.url, http_client=http) as (read, write, _),
            ClientSession(read, write) as session,
        ):
            await session.initialize()
...
class SqliteBank(BankObservation, BankLifecycle):
