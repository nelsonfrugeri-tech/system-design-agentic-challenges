...
if [[ ! -f "$runtime_env" ]]; then
  ...
  trap 'rm -f -- "$temporary_env"' EXIT
  {
    ...
    printf 'LANGFUSE_INIT_USER_EMAIL=experiment@localhost.local\n'
    printf 'LANGFUSE_PUBLIC_KEY=pk-lf-%s\n' "$(openssl rand -hex 16)"
    printf 'LANGFUSE_SECRET_KEY=sk-lf-%s\n' "$(openssl rand -hex 32)"
    for variable in POSTGRES_PASSWORD CLICKHOUSE_PASSWORD REDIS_AUTH MINIO_ROOT_PASSWORD SALT ENCRYPTION_KEY NEXTAUTH_SECRET LANGFUSE_INIT_USER_PASSWORD; do
